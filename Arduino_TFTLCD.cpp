...
void Arduino_TFTLCD::colorPixel(uint16_t y, uint16_t color) {
    uint8_t line_flag = FRAME_ID_FLAG8*(y&1);
    uint8_t mask_test = mask_flag^line_flag;
    if (do_masking) {
        START_READING();
        DELAY1
        uint8_t R = QUICK_READ;
        STOP_READING();
        if ((R&FRAME_ID_FLAG8)==mask_test) return;
    } else {
...
void Arduino_TFTLCD::drawFastHLine(int16_t x, int16_t y, int16_t length, uint16_t color){
    ...
    #endif
    uint8_t line_flag = FRAME_ID_FLAG8*(y&1);
    uint8_t mask_test = mask_flag^line_flag;//^FRAME_ID_FLAG8;
...
void Arduino_TFTLCD::fastLine(
    ...
    if (dy>dx) {
        ...
        START_PIXEL_DATA();
        while (y0<=y1) {
            PORTB=PORTD=color|mask_flag^(FRAME_ID_FLAG8*(y0&1));
            CLOCK_1; 
            y0++;
            err -= dx;
            if (err < 0) {
