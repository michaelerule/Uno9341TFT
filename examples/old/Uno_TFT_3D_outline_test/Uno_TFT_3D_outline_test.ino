...
void model() {
  ...
  while (1) 
  {
    ...
    const uint8_t NBYTES = (NTRIANGLES>>3)+1;
    uint8_t normal_orientations[NBYTES];
    for (i=0; i<NBYTES; i++) normal_orientations[i]=0;
    for (i=0; i<NTRIANGLES; i++) {
      ...
      if ((int)(r[0]-p[0])*(q[1]-p[1])<(int)(q[0]-p[0])*(r[1]-p[1])) {
        normal_orientations[i>>3]|=1<<(i&7);
      }
    }

    // Enumerate all edges
    for (i=0; i<NEDGES; i++) {
      uint8_t t1 = pgm_read_byte(&edgemap[i*4+2]);
      uint8_t t2 = pgm_read_byte(&edgemap[i*4+3]);
      uint8_t n1 = (normal_orientations[t1>>3]>>(t1&7))&1;
      uint8_t n2 = (normal_orientations[t2>>3]>>(t2&7))&1;
      if (n1!=n2) {
        uint8_t ip = pgm_read_byte(&edgemap[i*4+0]);
        uint8_t iq = pgm_read_byte(&edgemap[i*4+1]);
    ...
    // Draw edges from this frame. 
    tft.masking_on = 0;
    color = 0b0011000111111111 | tft.mask_flag*0x0100;
    uint8_t *edges = (uint8_t *)edgeset1;
    for (uint8_t e=0; e<nset1; e++) {
      uint8_t *edge = &edges[e*2];
      uint8_t a = edge[0];
      ...
      int8_t *p = &vbuff1[a*2];
      int8_t *q = &vbuff1[b*2];
      tft.dashedLine(p[0]+X0,p[1]+Y0,q[0]+X0,q[1]+Y0,color,1);
    }
    ...
    for (uint8_t e=0; e<nset2; e++) {
      ...
      int8_t *p = &vbuff2[a*2];
      int8_t *q = &vbuff2[b*2];
      tft.dashedLine(p[0]+X0,p[1]+Y0,q[0]+X0,q[1]+Y0,WHITE,1);
    }
