...
void drawFastColorPallet() {  
    ...
    for (int i=0; i<12; i++) {
        uint8_t c = pallet[i];       
        uint8_t x = (i/3)*PALLET_SIZE+PALLET_X+2;
        uint8_t y = (i%3)*PALLET_SIZE+PALLET_Y+2;
        uint8_t l = PALLET_SIZE-4;
        tft.fastFillRect(x,y,l,l,c);
        tft.drawRect(x-1,y-1,l+2,l+2,BLACK);
    }
}
