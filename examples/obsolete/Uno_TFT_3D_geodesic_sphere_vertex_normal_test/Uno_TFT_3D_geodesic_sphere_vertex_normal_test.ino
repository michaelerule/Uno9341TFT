...
void model() {
  ...
  while (1) 
  {
    ...
    for (i=0; i<NTRIANGLES; i++) {
      ...
      normal = &vertexnormals[pi*3];
      nx = (int8_t)pgm_read_byte(&normal[0]);
      ny = (int8_t)pgm_read_byte(&normal[1]);
      nz = (int8_t)pgm_read_byte(&normal[2]);
      lx = nx*abuff1[0]+ny*abuff1[3]+nz*abuff1[6];
      ly = nx*abuff1[1]+ny*abuff1[4]+nz*abuff1[7];
      lz = nx*abuff1[2]+ny*abuff1[5]+nz*abuff1[8];
      if (lx<0) lx=0;
      if (ly<0) ly=0;
      if (lz<0) lz=0;
      uint16_t color1 = (0b0000100000000001*(lx>>3) + 0b0000100001000000*(ly>>3) + 0b0000000001000001*(lz>>3))&0b1111011111111111 | tft.mask_flag*0x0100;
      
      normal = &vertexnormals[qi*3];
      nx = (int8_t)pgm_read_byte(&normal[0]);
      ny = (int8_t)pgm_read_byte(&normal[1]);
      nz = (int8_t)pgm_read_byte(&normal[2]);
      lx = nx*abuff1[0]+ny*abuff1[3]+nz*abuff1[6];
      ...
      if (ly<0) ly=0;
      if (lz<0) lz=0;
      uint16_t color2 = (0b0000100000000001*(lx>>3) + 0b0000100001000000*(ly>>3) + 0b0000000001000001*(lz>>3))&0b1111011111111111 | tft.mask_flag*0x0100;
      
      normal = &vertexnormals[ri*3];
      nx = (int8_t)pgm_read_byte(&normal[0]);
      ny = (int8_t)pgm_read_byte(&normal[1]);
      nz = (int8_t)pgm_read_byte(&normal[2]);
      lx = nx*abuff1[0]+ny*abuff1[3]+nz*abuff1[6];
      ...
      if (ly<0) ly=0;
      if (lz<0) lz=0;
      uint16_t color3 = (0b0000100000000001*(lx>>3) + 0b0000100001000000*(ly>>3) + 0b0000000001000001*(lz>>3))&0b1111011111111111 | tft.mask_flag*0x0100;
