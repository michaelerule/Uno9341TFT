  ...
  while (1) 
  {
    ...
    for (int j=0; j<NTRIANGLES; j++) {
      ...
      if ((int)(r[0]-p[0])*(q[1]-p[1])<(int)(q[0]-p[0])*(r[1]-p[1])) {
        uint8_t color1 = normal_colors[pi]/12;
        uint8_t color2 = normal_colors[qi]/12;
        uint8_t color3 = normal_colors[ri]/12;
        tft.fastShadeTriangle(p[0]+X0,p[1]+Y0,q[0]+X0,q[1]+Y0,r[0]+X0,r[1]+Y0,color1,color2,color3);
      }
    }
