...
// Using fast but less accurate drawing commands
// This does not used masked erasing on the 3D cube demo at the end
// (so it flickers slightly). 

// This demo is designed to be compiled with the flag DISABLE_MASKING_AND_OVERDRAW set in Arduino_TFTLCD.cpp
// This will disable masked erasing and overdraw checking, which breaks the 3D rendering
// But! the cube model is so simple and drawing so fast, it does not matter. 
...
unsigned long testFillScreen() {
  ...
  tft.fastFillScreen(RED);
  tft.fastFillScreen(GREEN);
  tft.fastFillScreen(BLUE);
  tft.fastFillScreen(BLACK);
  return micros() - start;
}
...
unsigned long testLines(uint16_t color) {
  ...
  y2    = h - 1;
  start = micros();
  for(x2=0; x2<w; x2+=6) tft.fastLine(x1, y1, x2, y2, color);
  x2    = w - 1;
  for(y2=0; y2<h; y2+=6) tft.fastLine(x1, y1, x2, y2, color);
...
unsigned long testFastLines(uint16_t color1, uint16_t color2) {
  ...
  tft.fastFillScreen(BLACK);
  start = micros();
  for(y=0; y<h; y+=5) tft.fastestHLine(0, y, w, color1);
  for(x=0; x<w; x+=5) tft.fastestVLine(x, 0, h, color2);
  return micros() - start;
}
...
unsigned long testRects(uint16_t color) {
  ...
  for(i=2; i<n; i+=6) {
    i2 = i / 2;
    tft.fastDrawRect(cx-i2, cy-i2, i, i, color);
  }
  return micros() - start;
}
...
unsigned long testFilledRects(uint16_t color1, uint16_t color2) {
  ...
  for(i=n; i>0; i-=6) {
    i2    = i / 2;
    start = micros();
    tft.fastFillRect(cx-i2, cy-i2, i, i, color1);
    t    += micros() - start;
    // Outlines are not included in timing results
    tft.fastDrawRect(cx-i2, cy-i2, i, i, color2);
  }
  return t;
}
...
unsigned long testFilledTriangles() {
  ...
  for(i=min(cx,cy); i>10; i-=5) {
    start = micros();
    tft.fastFillTriangle(cx, cy-i, cx-i, cy+i, cx+i, cy+i,
      color565(0, i, i));
    t += micros() - start;
    tft.fastDrawTriangle(cx, cy-i, cx-i, cy+i, cx+i, cy+i,
      color565(i, i, 0));
  }
...
void cube() {
  ...
  while (1) 
  {
    ...
    // Solution that does not use masking
    // This should flicker a bit
    tft.masking_off();

    // Update axis and erase old axis
    ...
    for (int j=0; j<3; j++) {
      ...
      tft.fastLine(x0,y0,x+x0,y+y0,BLACK);
      tft.fastLine(x0,y0,nx+x0,ny+y0,colors[j]);
    }
    ...
    for (i=0; i<NEDGES; i++) {
      ...
      tft.fastLine(p[0],p[1],q[0],q[1],BLACK);
      // Draw the next frame of the cube
      p = &vbuff1[pgm_read_byte(&e[0])*2];
      q = &vbuff1[pgm_read_byte(&e[1])*2];
      tft.fastLine(p[0],p[1],q[0],q[1],WHITE);
    }
