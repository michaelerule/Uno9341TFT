...
The 9341 touch breakout is limited in a number of ways. The pin for 
synchronizing display updates with display refresh is not broken out. 
However, this would make little difference as the AtMega in general does
not have enough computational power to update the display in a single frame.
The amount of video RAM on the LCD driver is at least 75x the amount of 
RAM on the AtMega, meaning that double buffering on the microcontroller
side is simply impossible without additional memory peripherals. 
...
This file is derived from Uno's TFT driver library, with substantial
...
Features: 
    Optimized for fast communication (~10x+ speedups)
    ...
    Implemented features for advanced rendering:
        ...
        A limited color pallet of "fast colors" that can be sent with fewer
            instructions. 
    Added support for specific features of ID_9341 including
        Entering and exiting 3-bit color mode
...
This file is derived from Uno's driver. Please support Uno by
